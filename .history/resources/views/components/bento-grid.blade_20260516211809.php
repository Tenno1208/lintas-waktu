...
<section x-data="{ 
    ...
    // Fungsi bantuan untuk ngecek apakah ada data yang tampil di layar berdasarkan filter
    hasVisibleItems() {
    ...
    <div class="flex flex-wrap gap-2 mb-8 border-b border-zinc-950 pb-4 overflow-x-auto select-none">
        <button 
            ...
            :class="activeFilter === 'all' ? 'bg-zinc-100 text-zinc-900 font-bold' : 'bg-zinc-950 text-zinc-500 border border-zinc-900 hover:border-zinc-700'"
            class="px-4 py-1.5 rounded-full text-[10px] font-mono uppercase transition-colors duration-300"
        >
            All Stories
        </button>
        <button 
            @click="activeFilter = 'adventure'"
            :class="activeFilter === 'adventure' ? 'bg-emerald-500 text-black font-bold' : 'bg-zinc-950 text-zinc-500 border border-zinc-900 hover:border-zinc-700'"
            class="px-4 py-1.5 rounded-full text-[10px] font-mono uppercase transition-colors duration-300"
        >
            🗻 Adventure
        </button>
        <button 
            ...
            :class="activeFilter === 'school' ? 'bg-blue-500 text-white font-bold' : 'bg-zinc-950 text-zinc-500 border border-zinc-900 hover:border-zinc-700'"
            class="px-4 py-1.5 rounded-full text-[10px] font-mono uppercase transition-colors duration-300"
        >
            🏫 School Days
        </button>
        <button 
            @click="activeFilter = 'graduation'"
            :class="activeFilter === 'graduation' ? 'bg-amber-500 text-black font-bold' : 'bg-zinc-950 text-zinc-500 border border-zinc-900 hover:border-zinc-700'"
            class="px-4 py-1.5 rounded-full text-[10px] font-mono uppercase transition-colors duration-300"
        >
            🎓 Graduation
        </button>
    ...
    <div 
        ...
        x-transition:enter-start="opacity-0 transform scale-95"
        x-transition:enter-end="opacity-100 transform scale-100"
        class="col-span-full py-24 text-center border border-dashed border-zinc-900 rounded-[2rem] bg-[#070709]/40 select-none"
        style="display: none;"
    >
