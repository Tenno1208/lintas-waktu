...
<section x-data="{ 
    ...
    // INOVASI 5: Logic Gali Kapsul Acak (Random Capsule)
    openRandomCapsule() {
        ...
        // Jika yang terambil kategori graduation, pemicu confetti sekalian!
        if (this.selected.category === 'graduation') {
            setTimeout(() => { this.triggerGraduationConfetti(); }, 300);
        }
    },

    // INOVASI 3: Efek Selebrasi Lempar Topi Wisuda
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-8 border-b border-zinc-950 pb-4">
        ...
        <button @click="openRandomCapsule()" class="w-full md:w-auto bg-zinc-950 border border-zinc-900 hover:border-emerald-500/40 text-zinc-400 hover:text-emerald-400 px-4 py-2 rounded-xl text-[10px] font-mono tracking-widest transition-all duration-300 flex items-center justify-center gap-2">
            <svg class="w-3.5 h-3.5 animate-spin-slow" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
            GALI KAPSUL ACAK
        </button>
    </div>
                ...
                <div class="w-full md:w-8/12 h-[50vh] md:h-full bg-[#020202] relative flex flex-col justify-between p-4 border-b md:border-b-0 md:border-r border-zinc-900 select-none">
                    <div class="w-full flex-1 flex items-center justify-center relative overflow-hidden">
                        ...
                        <template x-if="selected && selected.photos.length > 1">
                            <div>
                                <button @click="prevPhoto()" class="absolute left-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white p-2.5 rounded-xl transition-all backdrop-blur-xs"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"/></svg></button>
                                <button @click="nextPhoto()" class="absolute right-2 top-1/2 -translate-y-1/2 bg-black/40 hover:bg-zinc-900 border border-zinc-800 text-zinc-400 hover:text-white p-2.5 rounded-xl transition-all backdrop-blur-xs"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg></button>
                            </div>
                        </template>
                    </div>
                ...
                <div class="w-full md:w-4/12 p-6 md:p-10 overflow-y-auto flex flex-col justify-between h-[50vh] md:h-full bg-[#050505]">
                    <div>
                        <div class="flex justify-between items-start mb-6">
                            ...
                            <button @click="open = false" class="text-zinc-500 hover:text-zinc-200 border border-zinc-900 p-1.5 rounded-lg transition-colors bg-zinc-950/40"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12"/></svg></button>
                        </div>
