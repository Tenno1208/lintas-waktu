...
Route::prefix('admin')
    ...
    return app()->make(App\Http\Controllers\AdminController::class)->store($request);
        ...
        Route::match(['put', 'get'], '/{id}/update', function(Illuminate\Http\Request $request, $id) {
    if ($request->isMethod('get')) {
        // Kalau mental jadi GET, balikin langsung ke halaman form edit biar gak error merah
        return redirect()->route('admin.edit', $id);
    }
    return app()->make(App\Http\Controllers\AdminController::class)->update($request, $id);
})->name('admin.update');
    });
